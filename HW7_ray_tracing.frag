...
void initScene() 
{
    ...
    materials[1].kd = vec3(0.8, 0.1, 0.1); 
    ...
    materials[3].kd = vec3(0.2, 0.2, 0.2);
    ...
    spheres[0] = Sphere(vec3(0, 0.6, -1), 0.8, 1);
    spheres[1] = Sphere(vec3(1.1, 0.4, -0.8), 0.6, 2);
    spheres[2] = Sphere(vec3(-1.1, 0.4, -0.8), 0.6, 2);
}
...
Hit hitSphere(const Ray r, const Sphere s) 
{
    ...
    // find the vector from the center of the sphere to ray's origin
    vec3 oc = r.ori - s.ori;

    // find a, b, c coefficients for the quadratic equation
    ...
    if (discriminant >= 0.0) {
        ...
        if (t > 0.0) {
            // find the intersection point and normal
            vec3 hitP = r.ori + t * r.dir;
            vec3 normal = normalize(hitP - s.ori);
            
            // make a hit structure with intersection properties
...
vec3 shadingPhong(Light light, int matId, vec3 e, vec3 p, vec3 s, vec3 n) 
{
    ...
    vec3 diffuse_color = (matId == 0) ? sampleDiffuse(matId, p) : materials[matId].kd;

    // find the vectors for Phong shading
    vec3 v = normalize(e - p); // viewer
    vec3 l = normalize(s - p); // light source
    vec3 r = reflect(-l, n);   // reflection

    ka = materials[matId].ka;
    ks = materials[matId].ks;
    shininess = materials[matId].shininess;

    vec3 ambient_reflection = light.Ia * ka;
    vec3 diffuse_reflection = light.Id * diffuse_color * max(0.0, dot(n, l));
    vec3 specular_reflection = light.Is * ks * pow(max(0.0, dot(v, r)), shininess);

    // combine reflections
    color = ambient_reflection + diffuse_reflection + specular_reflection;
...
vec3 sampleDiffuse(int matId, vec3 p) 
{
    ...
    if(matId == 0) {		
        ...
        vec3 texture_color = texture(floor_color, uv).rgb;
        color = texture_color * mat_color;
...
bool isShadowed(Light light, Hit h) 
{
    ...
    intersect += Epsilon * dir;

    // make the shadow ray
    Ray shadowRay = Ray(intersect, dir);

    // check if the shadow ray intersects with objects in the scene
...
void main() 
{
    ...
    for(int i = 0; i < recursiveDepth; i++) {
        ...
        intersect += Epsilon * reflected_dir;
