...
vec3 renderParticle(vec2 fragPos, vec2 particlePos, float brightness, vec3 color)
{
    ...
    if (distance > 0.0) {
        ...
        fragColor = brightness * decay * color;
    }
...
vec3 renderStars(vec2 fragPos)
{
    ...
    for(float i = 0.; i < NUM_STAR; i++){
        ...
        pos = hash2d(i + t) * 2. - 1.;
        brightness = abs(sin(t * 0.5)) * 0.0004;
...
vec2 moveParticle(vec2 initPos, vec2 initVel, float t)
{
    ...
    vec2 displacement = initVel * t;
    vec2 gravityDisplacement = 0.5 * g * t * t;
    currentPos += displacement + gravityDisplacement;
    initPos = currentPos;
...
vec3 simSingleFirework(vec2 fragPos, vec2 launchPos, vec2 launchVel, float t, vec3 color)
{
    ...
    if(t < emitTime){
        ...
        for(float i = 0.; i < NUM_EMISSION; i++){
            ...
            vec2 emitDir = normalize(hash2d(i + t));
            vec2 emitVel = emitDir * length(launchVel) * (hash1d(i + t) + 0.25);

            float brightness = abs(sin(emitT * 20.0 + i * 0.5)) * 0.002;
