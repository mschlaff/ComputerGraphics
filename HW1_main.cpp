...
const std::string author="Madison Schlaff";
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	// background sky
	vec3 color = vec3(0.599, 0.815, 0.950);

	//background ice
	if (distance(vec2(0.5, 0.5), vec2(st.x, st.y * 5)) < 1.2) {
		color = vec3(0.9, 1.0, 1.0);
	}

	// left foot orange
	if (distance(vec2(0.6, 0.2), vec2(st.x * 1.5, st.y)) < 0.1) {
		color = vec3(1.00, 0.5, 0.02);
	}

	//right foot orange
	if (distance(vec2(0.9, 0.2), vec2(st.x * 1.5, st.y)) < 0.1) {
		color = vec3(1.00, 0.5, 0.02);
	}

	// black body
	if (distance(vec2(0.9, 0.5), vec2(st.x * 1.8, st.y)) < 0.3) {
		color = vec3(0.09, 0.09, 0.09);
	}

	// black left arm
	if (distance(vec2(0.73, 0.52), vec2(st.x * 1.8, st.y)) < 0.2) {
		color = vec3(0.09, 0.09, 0.09);
	}

	// black right arm
	if (distance(vec2(1.07, 0.52), vec2(st.x * 1.8, st.y)) < 0.2) {
		color = vec3(0.09, 0.09, 0.09);
	}

	// stomach yellow
	if (distance(vec2(1.0, 0.5), vec2(st.x * 2, st.y)) < 0.2) {
		color = vec3(1.00, 0.749, 0.185);
	}

	// stomach white
	if (distance(vec2(1.0, 0.48), vec2(st.x * 2, st.y)) < 0.19) {
		color = vec3(0.96, 0.96, 0.96);
	}

	// black head
	if (distance(vec2(0.6, 0.85), vec2(st.x * 1.2, st.y)) < 0.12) {
		color = vec3(0.09, 0.09, 0.09);
	}

	// white left eye
	if (distance(vec2(0.67, 0.87), vec2(st.x * 1.4, st.y)) < 0.05) {
		if (st.y - 0.85 < abs(sin(iTime * 1.5)) * 0.15) {
			color = vec3(1.0, 1.0, 1.0);
		}
	}

	// white right eye
	if (distance(vec2(0.73, 0.87), vec2(st.x * 1.4, st.y)) < 0.05) {
		if (st.y - 0.85 < abs(sin(iTime * 1.5)) * 0.15) {
			color = vec3(1.0, 1.0, 1.0);
		}
	}
	...
	// black left eye
	if (distance(vec2(0.98, 0.865), vec2(st.x * 2, st.y)) < 0.02) {
		if (st.y - 0.85 < abs(sin(iTime * 1.5)) * 0.15) {
			color = vec3(0.035, 0.026, 0.006);
		}
	}

	// black right eye
	if (distance(vec2(1.02, 0.865), vec2(st.x * 2, st.y)) < 0.02) {
		if (st.y - 0.85 < abs(sin(iTime * 1.5)) * 0.15) {
			color = vec3(0.035, 0.026, 0.006);
		}
	}
	...
	// orange mouth
	if (distance(vec2(0.5, 1.38), vec2(st.x, st.y * 1.7)) < 0.05) {
		color = vec3(1.0, 0.5, 0.02);
	}

	// black mouth
	if (distance(vec2(0.52, 1.36), vec2(st.x, st.y * 1.7)) < 0.04 / M_PI) {
		color = vec3(0.09, 0.09, 0.09);
	}
