...
vec4 shading_texture_with_checkerboard() 
{
    ...
    // find grid cell indices from uv coords
    int cellX = int(uv.x / cellSize);
    int cellY = int(uv.y / cellSize);

    //find olor based on cells. odd cell = black, even cell = white
    if ((cellX + cellY) % 2 == 0) {
        color = vec3(1.0);
    } else {
        color = vec3(0.0);
...
vec4 shading_texture_with_phong(Light light, vec3 e, vec3 p, vec3 s, vec3 n) 
{
    ...
    /* your implementation starts */
    vec3 diffuse = kd * light.Id * max(0.0, dot(n, normalize(s - p))) * tex_color;

    // Phong specular term
    vec3 v = normalize(e - p);                              // View direction
    vec3 h = normalize(v + normalize(s - p));               // Half vector
    float specularTerm = pow(max(0.0, dot(n, h)), shininess);
    vec3 specular = ks * light.Is * specularTerm;

    // Final Phong shading color with texture
...
vec3 read_normal_texture() 
{
    ...
    normal = texture(tex_normal, uv).xyz;
